...
const versionNum = eval(vscode_1.version.replace('.', ' * 10000 + ').replace('.', ' * 100 + ').replace('-insider', ' - 1'));
...
if (versionNum < 12400) {
    ...
    fromHeading = require(path.join(exports.officialExtPath, 'out', 'slugify')).githubSlugifier.fromHeading;
}
...
function slugify(heading) {
    try {
        ...
        vscode_1.window.showWarningMessage(localize_1.default("cannotUseBuiltinSlugifyFunc"));
    }
...
function previewChangelog() {
    vscode_1.commands.executeCommand('markdown.showPreview', vscode_1.Uri.file(path.join(__dirname, '../../CHANGELOG.md')));
}
