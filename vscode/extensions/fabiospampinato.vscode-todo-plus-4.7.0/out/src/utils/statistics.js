...
const statistics_tokens_1 = require("./statistics_tokens");
const time_1 = require("./time");
/* STATISTICS */
const Statistics = {
    /* TIME TAGS */
    timeTags: {
        add(tag, tokens, disabledTokens, disabledEst = false) {
            const prefix = tag[1];
        ...
        elapseds: {},
        parseElapsed(tag) {
            if (Statistics.timeTags.elapseds[tag])
                return Statistics.timeTags.elapseds[tag];
            const match = tag.match(consts_1.default.regexes.tagElapsed);
            if (!match)
                return 0;
            const time = match[1], seconds = time_1.default.diffSeconds(time);
            Statistics.timeTags.elapseds[tag] = seconds;
            return seconds;
        },
        estimates: {},
        parseEstimate(tag, from) {
            if (Statistics.timeTags.estimates[tag])
                return Statistics.timeTags.estimates[tag];
            const est = tag.match(consts_1.default.regexes.tagEstimate);
            if (!est)
                return 0;
    ...
    tokens: {
        ...
        updateDisabled(obj, tokens, settings) {
            tokens.forEach(token => {
                obj[token] = !settings.find(setting => _.includes(config_1.default.getKey(setting), token));
            });
        },
        global: {},
        updateGlobal(items) {
            ...
            const tokens = _.extend(new statistics_tokens_1.default(), {
        ...
        updateProject(project, lines, lineNr) {
            ...
            const tokens = new statistics_tokens_1.default();
    ...
    template: {
        ...
        render(template, tokens = Statistics.getTokens()) {
            if (!tokens)
                return;
            for (let token of statistics_tokens_1.default.supported) {
