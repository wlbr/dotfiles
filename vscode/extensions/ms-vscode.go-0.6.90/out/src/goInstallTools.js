    ...
    missing.reduce((res, tool) => {
        return res.then(sofar => new Promise((resolve, reject) => {
            const callback = (err, stdout, stderr) => {
                if (err) {
                    goStatus_1.outputChannel.appendLine('Installing ' + allTools[tool] + ' FAILED');
                    let failureReason = tool + ';;' + err + stdout.toString() + stderr.toString();
                    resolve([...sofar, failureReason]);
                }
                else {
                    goStatus_1.outputChannel.appendLine('Installing ' + allTools[tool] + ' SUCCEEDED');
            ...
            if (tool === 'gocode' || tool === 'gocode-gomod') {
                closeToolPromise = new Promise((innerResolve) => {
                    cp.execFile(util_1.getBinPath(tool), ['close'], {}, (err, stdout, stderr) => {
