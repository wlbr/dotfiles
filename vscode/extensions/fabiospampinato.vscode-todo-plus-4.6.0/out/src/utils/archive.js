...
const Archive = {
    ...
    edit(doc, data) {
        return __awaiter(this, void 0, void 0, function* () {
            const finishedFormat = config_1.default.getKey('timekeeping.finished.format');
            let prevFinishedDate = -1; // Ensuring comments' position relative to their parent todo is preserved
            function getFinishedDate(line) {
                if (consts_1.default.regexes.todoFinished.test(line)) {
                    const match = line.match(consts_1.default.regexes.tagFinished);
                    if (match)
                        return prevFinishedDate = moment(match[1], finishedFormat).toDate();
                    return prevFinishedDate = -1;
                }
                return prevFinishedDate;
            }
            const line2number = line => line.lineNumber, line2date = config_1.default.getKey('archive.sortByDate') ? line => getFinishedDate(line.text) : _.constant(-1), natSort = (a, b) => a.lineNumber - b.lineNumber, removeLines = _.uniqBy(data.remove, line2number), //TSC
    ...
    transformations: {
        order: ['addTodosFinished', 'addTodosComments', 'addProjectTag', 'removeEmptyProjects', 'removeEmptyLines'],
        addTodosFinished(doc, data) {
            const todosFinished = doc.getTodosFinished(), lines = todosFinished.map(todo => todo.line);
            lines.forEach(line => {
