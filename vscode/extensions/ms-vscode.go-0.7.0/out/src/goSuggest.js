...
class GoCompletionItemProvider {
    ...
    resolveCompletionItem(item, token) {
        ...
        return util_1.runGodoc(item.package || path.dirname(item.fileName), item.label, token).then(doc => {
            ...
            return new Promise((resolve, reject) => {
                ...
                return this.runGoCode(document, filename, inputText, offset, inString, position, lineText, currentWord, includeUnimportedPkgs, config).then(suggestions => {
                    ...
                    if (suggestions.length === 0 && lineTillCurrentPosition.endsWith('.')) {
                        ...
                        if (pkgPath.length > 1) {
                            pkgPath.forEach(pkg => {
                                let item = new vscode.CompletionItem(`${lineTillCurrentPosition.replace('.', '').trim()} (${pkg})`, vscode.CompletionItemKind.Module);
    ...
    runGoCode(document, filename, inputText, offset, inString, position, lineText, currentWord, includeUnimportedPkgs, config) {
        return new Promise((resolve, reject) => {
            ...
            let gocode = util_1.getBinPath(gocodeName);
    ...
    ensureGoCodeConfigured(fileuri) {
        ...
        let setPkgsList = goPackages_1.getImportablePackages(currentFile, true).then(pkgMap => { this.pkgsList = pkgMap; });
        if (!this.setGocodeOptions) {
