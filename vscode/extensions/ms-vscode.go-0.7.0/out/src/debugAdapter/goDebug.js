    ...
    constructor(remotePath, port, host, program, launchArgs) {
        ...
        this.connection = new Promise((resolve, reject) => {
            ...
            if (!fs_1.existsSync(dlv)) {
                verbose(`Couldn't find dlv at ${process.env['GOPATH']}${env['GOPATH'] ? ', ' + env['GOPATH'] : ''} or ${goPath_1.envPath}`);
...
class GoDebugSession extends vscode_debugadapter_1.DebugSession {
    ...
    stackTraceRequest(response, args) {
        ...
        this.delve.call(this.delve.isApiV1 ? 'StacktraceGoroutine' : 'Stacktrace', [stackTraceIn], (err, out) => {
            ...
            let stackFrames = locations.map((location, i) => new vscode_debugadapter_1.StackFrame(i, location.function ? location.function.name : '<unknown>', location.file === '<autogenerated>' ? null : new vscode_debugadapter_1.Source(path_1.basename(location.file), this.toLocalPath(location.file)), location.line, 0));
            ...
            this.delve.call('ListFunctionArgs', this.delve.isApiV1 ? [listLocalFunctionArgsIn] : [{ scope: listLocalFunctionArgsIn, cfg: this.delve.loadConfig }], (err, outArgs) => {
                ...
                this.getPackageInfo(this.debugState).then(packageName => {
                    ...
                    this.delve.call('ListPackageVars', this.delve.isApiV1 ? [filter] : [{ filter, cfg: this.delve.loadConfig }], (err, out) => {
                        ...
                        scopes.push(new vscode_debugadapter_1.Scope('Global', this._variableHandles.create(globalVariables), false));
                        this.sendResponse(response);
                        verbose('ScopesResponse');
                    });
                });
