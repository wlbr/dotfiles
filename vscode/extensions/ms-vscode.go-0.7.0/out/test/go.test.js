    ...
    function testHoverProvider(goConfig, testCases) {
        ...
        return vscode.workspace.openTextDocument(uri).then((textDocument) => {
            let promises = testCases.map(([position, expectedSignature, expectedDocumentation]) => provider.provideHover(textDocument, position, null).then(res => {
                ...
                assert.equal(expectedDocumentation, (res.contents[1]));
            }));
            return Promise.all(promises);
        }, (err) => {
            assert.ok(false, `error in OpenTextDocument ${err}`);
...
`;
    ...
    test('Test Env Variables are passed to Tests', (done) => {
        ...
        vscode.workspace.openTextDocument(uri).then(document => {
            return vscode.window.showTextDocument(document).then(editor => {
                return goTest_1.testCurrentFile(config, false, []).then((result) => {
                    assert.equal(result, true);
                    return Promise.resolve();
                });
            });
    ...
    test('Test Outline', (done) => {
        ...
        let options = { fileName: filePath, importsOption: goOutline_1.GoOutlineImportsOptions.Include, skipRanges: true };
    ...
    test('Test Outline imports only', (done) => {
        let filePath = path.join(fixturePath, 'outlineTest', 'test.go');
        let options = { fileName: filePath, importsOption: goOutline_1.GoOutlineImportsOptions.Only, skipRanges: true };
        goOutline_1.documentSymbols(options, null).then(outlines => {
            let packageSymbols = outlines.filter(x => x.kind === vscode.SymbolKind.Package);
            let imports = outlines.filter(x => x.kind === vscode.SymbolKind.Namespace);
            let functions = outlines.filter(x => x.kind === vscode.SymbolKind.Function);
    ...
    test('Test Outline document symbols', (done) => {
        ...
        vscode.workspace.openTextDocument(uri).then(document => {
            new goOutline_1.GoDocumentSymbolProvider().provideDocumentSymbols(document, null).then(symbols => {
...
`;
        ...
        vscode.workspace.openTextDocument(uri).then((textDocument) => {
            return vscode.window.showTextDocument(textDocument).then(editor => {
                let promises = testCases.map(([position, expectedLabel, expectedDetail, expectedDoc]) => provider.provideCompletionItems(editor.document, position, null).then(items => {
