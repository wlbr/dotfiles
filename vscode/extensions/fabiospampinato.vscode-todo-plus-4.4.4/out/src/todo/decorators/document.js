    ...
    updateLines(res = vscode.window.activeTextEditor, lineNrs) {
        ...
        if (prevLines && prevLines.length === doc.textDocument.lineCount) {
            ...
            const isUnchanged = lineNrs.every(lineNr => {
                const prevLine = prevLines[lineNr], prevDoc = new document_1.default(prevLine), prevItems = Document.getItems(prevDoc), currLine = doc.textDocument.lineAt(lineNr).text, currDoc = new document_1.default(currLine), currItems = Document.getItems(currDoc);
